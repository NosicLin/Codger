...
#else
extern void __list_add(struct list_head *new_list,
			      struct list_head *prev,
			      struct list_head *next);
#endif
...
/**
 * list_replace - replace old entry by new_list one
 * @old : the element to be replaced
 * @new_list : the new_list element to insert
 *
...
{
	struct list_head *new_list_first = entry->next;
	list->next = head->next;
	list->next->prev = list;
	list->prev = entry;
	entry->next = list;
	head->next = new_list_first;
	new_list_first->prev = head;
}

/**
 * list_cut_position - cut a list into two
 * @list: a new_list list to add all removed entries
